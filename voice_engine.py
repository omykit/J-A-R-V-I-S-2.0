...
class VoiceEngine:
    ...
    def _prepare_tts_text(self, text: str) -> str:
        ...
        cleaned = re.sub(r"\b(current|right now|currently|approximately|basically|actually|just|simply)\b", "", cleaned, flags=re.IGNORECASE)
        cleaned = re.sub(r"\b(in your local timezone)\b", "", cleaned, flags=re.IGNORECASE)
        cleaned = re.sub(r"\s+", " ", cleaned).strip(" ,.")
        ...
        cleaned = re.sub(r"(?i)\bplease note that\b", "", cleaned)
        cleaned = re.sub(r"(?i)\bi can tell you that\b", "", cleaned)
        cleaned = re.sub(r"(?i)\bit looks like\b", "", cleaned)
        cleaned = re.sub(r"(?i)\bfor your reference\b", "", cleaned)
        cleaned = re.sub(r"\s+", " ", cleaned).strip(" ,.")
        sentences = [
            sentence.strip(" ,.")
    ...
    def _is_intentional_interrupt(self, text: str) -> bool:
        normalized = self._normalize_match_text(text)
        if not normalized:
            return False
        if normalized in INTENTIONAL_INTERRUPT_PHRASES:
            return True
        words = normalized.split()
        if len(words) <= 1:
            return False
        return (
            ("jarvis" in words and any(word in words for word in ("stop", "wait")))
                    ...
                    while not self.stop_event.is_set():
                        if self.speaking_event.is_set():
                            ...
                            if self._looks_like_tts_echo(text, speaking_now=True):
                                ...
                                continue
                            if not self._is_intentional_interrupt(text):
                                self._log(f"tts_overlap_ignored:{text}")
                                continue
