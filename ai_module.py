...
def _mark_ai_healthy(logger: Logger | None = None) -> None:
    global _AI_HEALTH, ollama_available, ollama_degraded
    if _AI_HEALTH is None:
        return
    _AI_HEALTH = OllamaHealth(
        checked_at=time.perf_counter(),
        available=True,
        models=_AI_HEALTH.models,
        error="",
        degraded=False,
    )
    ollama_available = True
    ollama_degraded = False
    _safe_log(logger, "[AI] System marked healthy")
...
def get_ai_response_streaming(user_input: str, chat_history: list[dict], config: dict, logger: Logger | None = None) -> AIResponse:
    ...
    if not health.available:
        if health.degraded:
            _safe_log(logger, f"[AI] Rechecking degraded health before request: {health.error}")
            health = _check_ollama_health(base_url, timeout_seconds, logger)
            globals()["_AI_HEALTH"] = health
            globals()["ollama_available"] = health.available
            globals()["ollama_degraded"] = health.degraded
        if health.available:
            _safe_log(logger, "[AI] Degraded state recovered before request")
        else:
            if health.degraded:
    ...
    elif len(candidate_models) == 1 and MAX_RETRIES:
    ...
    if ai_response is None:
        ...
        if not _is_timeout_error(last_error):
            _mark_ai_degraded(str(last_error or "AI request failed"), logger)
        else:
            _safe_log(logger, f"[AI ERROR] Request timed out without marking system degraded: {last_error}")
        return _fallback_ai_response(user_input)
