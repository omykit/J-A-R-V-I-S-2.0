...
# Superseded by client/desktop_app.py (gateway-connected client) — kept
# intentionally as the working fallback until the new client reaches parity.
...
DEFAULT_CONFIG = {
    ...
    "ai": {
        ...
        "system_prompt": "You are Jarvis, Omair's personal local voice assistant. Be warm, direct, and useful. Use remembered facts and recent conversation when helpful. Answer naturally in one or two short sentences. Never claim an external action has been completed unless the application or tool explicitly reports success. Never claim an email was sent, an email reminder was scheduled, or an email notification was delivered unless a real email subsystem confirms it. Jarvis currently supports local spoken reminders, not email delivery.",
    },
    "weather": {
        "openweather_api_key": "",
...
WAKE_GREETINGS = [
    ...
    "Are we here to impress the interviewers{owner}.",
]
AI_ACK_DELAY_SECONDS = 5.0
...
def redact_log_message(message: str) -> str:
    ...
    redacted = re.sub(
        r"(?i)\b(my password is|password is|my token is|token is|my secret is|secret is)\s+[^,\n]+",
        r"\1 [REDACTED]",
        redacted,
    )
    return redacted
...
def log_event(message: str) -> None:
    try:
        with RUNTIME_LOG.open("a", encoding="utf-8") as handle:
            handle.write(redact_log_message(message) + "\n")
    except OSError:
        pass
...
def try_start_ai_request() -> int | None:
    global latest_ai_request_id, completed_ai_request_id, pending_ai_ack_request_id, pending_ai_ack_phrase, active_ai_request_id
    with request_state_lock:
        if active_ai_request_id:
            return None
        latest_ai_request_id += 1
        completed_ai_request_id = 0
        pending_ai_ack_request_id = 0
        pending_ai_ack_phrase = None
        active_ai_request_id = latest_ai_request_id
        return latest_ai_request_id
...
def mark_ai_request_complete(request_id: int) -> None:
    global completed_ai_request_id, active_ai_request_id
    with request_state_lock:
        if request_id == latest_ai_request_id:
            completed_ai_request_id = request_id
        if active_ai_request_id == request_id:
            active_ai_request_id = 0
...
def complete_ai_response_async(
    *,
    ...
    if completed_text != spoken_text:
        ...
        placeholder_responses = {"", "AHH let's what i have got for you hmm.", "Give me a moment... I'm processing it."}
...
def handle_ai_response_async(user_input: str, request_id: int) -> None:
    try:
        ...
        try:
            ...
            respond(
                "I ran into an AI response error. Please try again.",
...
def handle_text(text: str) -> None:
    ...
    if is_wake_phrase(normalized):
        invalidate_pending_ai_requests()
        jarvis_active = True
    ...
    request_id = try_start_ai_request()
    if request_id is None:
        log_event(f"ai_request_ignored_busy:{normalized}")
        update_ui(
            STATUS_THINKING,
            "AI request already in progress.",
            "System",
            subtitle="Ignoring overlapping speech while the current AI request finishes.",
            log_message=False,
        )
        return
