...
def test_settings_ignores_other_services_env_vars(monkeypatch):
    ...
    # (any prefix other than JARVIS_GW_) were present.
...
def test_settings_still_reads_its_own_env_vars(monkeypatch):
    monkeypatch.setenv("JARVIS_GW_ALLOWED_ORIGINS", "http://example.com,http://example.org")
    settings = Settings()
    assert settings.allowed_origins_list == ["http://example.com", "http://example.org"]
